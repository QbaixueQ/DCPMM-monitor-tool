...
!function ($) {
    $.fn.gaugeMeter = function (t) {
        var defaults = $.extend({
            ...
            prelabel: "",
            prelabel_color: "Black",
            desp: "",
            desp_color: "Black",
            label: "",           
            label_color: "Black",
            postlabel: "",
            postlabel_color: "Black",
            text: "",
            text_size: 0.22,
            fill: "",
            showvalue: false
        ...
        return this.each(function () {
            ...
            /* The prelabel below gauge. */
            function createPreLabel(t, a) {
                if (t.children("b").length === 0) {
                    $("<b></b>").appendTo(t).html(option.prelabel).css({
                        "line-height": option.size + -5 * a + "px",
                        color: option.prelabel_color
                    });
                    
                }
            }

            function createDesp(t, a) {
                if (t.children("b").length === 1) {
                    $("<b></b>").appendTo(t).html(option.desp).css({
                        "line-height": option.size + -3.5 * a + "px",
                        color: option.desp_color
                    });                    
                }
            }
            ...
            function createLabel(t, a) {
                if (t.children("b").length === 1) {
                    $("<b></b>").appendTo(t).html(option.label).css({
                        "line-height": option.size + 4 * a + "px",
                        "font-size": 15,
            ...
            /* The postlabel below gauge. */
            function createPostLabel(t, a) {
                if (t.children("b").length === 2) {
                    $("<b></b>").appendTo(t).html(option.postlabel).css({
                        "line-height": option.size + 6.5 * a + "px",
                        "font-size": 15,
                        color: option.postlabel_color
                    });
                }
            }
            ...
            function createSpanTag(t) {
                ...
                if (child.length !== 0) {
                    child.html(r).css({ color:option.span_color });
                    return;
                }
            ...
            function drawGauge(a) {
                // var m=b.width / 2-I, v=b.height+P * a - I;
                if (M < 0) M = 0;
                if (M > 100) M = 100;
                ...
                g.stroke();
                g.beginPath();
                // console.log(m, v, x, -I, P * a - I)
                // console.log(option.percent)
                // var d=b.width / 2+I*option.percent, f=b.height/2+(P * a - I);
                // var d=b.width / 2-I*10, f=b.height+(P * a - I);
                // console.log(d,f)
                // g.arc(d, f, x, -I, P - I, !1);
            ...
            createSpanTag(p);
            createPreLabel(p, option.size / 13);
            //createDesp(p, option.size / 13);
            createLabel(p, option.size / 13);
            createPostLabel(p, option.size / 13);
